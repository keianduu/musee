...
describe("import idempotency", () => {
  ...
  it("skips only a linked record with an unchanged checksum", () => { const existing = { id: "source-1", exhibition_id: "exhibition-1", checksum: "same" }; expect(shouldSkipExistingRecord(existing, "same")).toBe(true); expect(shouldSkipExistingRecord(existing, "changed")).toBe(false); expect(shouldSkipExistingRecord({ ...existing, exhibition_id: null }, "same")).toBe(false); });
